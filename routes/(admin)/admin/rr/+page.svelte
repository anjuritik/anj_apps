<script lang="ts">
	...
	async function saveVendorRemark({ ix, rr_no, vendor_remark }) {
		try {
			const res = await axios.post('api/rr', { rr_no, vendor_remark })
			if (res.status === 200) {
	...
	async function uploadCsvFile() {
		...
		try {
			const response = await axios.post('api/upload?t=rr', formData, {
				headers: {
